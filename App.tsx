      ...
      <AnimatePresence mode="wait">
        {showLanding ? (
          ...
          <motion.div
             ...
             transition={prefersReducedMotion ? { duration: 0 } : { duration: 0.8, ease: "circOut" }}
          ...
          >
            ...
            <motion.header
              ...
              animate={{ y: 0 }}
              transition={prefersReducedMotion ? { duration: 0 } : { type: "spring", stiffness: 300, damping: 30, delay: 0.3 }}
              ...
              <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 h-14 sm:h-16 md:h-20 flex items-center justify-between">
                <div className="flex items-center space-x-2 sm:space-x-3">
                  <motion.div
                    ...
                    className="w-8 h-8 sm:w-10 sm:h-10 bg-[#6FB92D] text-white font-bold rounded-lg sm:rounded-xl flex items-center justify-center text-lg sm:text-xl shadow-[0_0_15px_rgba(111,185,45,0.4)]"
                  >
                    M
                  </motion.div>
                  <h1 className="text-base sm:text-lg md:text-xl font-bold tracking-tight">{t('app.title', { highlight: 'AI' }).split('AI')[0]}<span className="text-[#6FB92D]">AI</span>{t('app.title', { highlight: 'AI' }).split('AI')[1]}</h1>
                </div>
                <div className="flex items-center space-x-3 sm:space-x-6">
                  <button
                    onClick={() => setShowAnalysis(true)}
                    className="min-w-touch min-h-touch p-2 sm:p-0 text-sm font-medium text-gray-400 hover:text-[#6FB92D] transition-colors flex items-center gap-2 tap-highlight-none no-select"
                  >
                  ...
                  <div className="relative group">
                    <motion.button
                      ...
                      className="min-w-touch min-h-touch w-9 h-9 sm:w-9 sm:h-9 rounded-full bg-[#6FB92D]/20 flex items-center justify-center text-[#6FB92D] font-bold border border-[#6FB92D]/30 cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#6FB92D]/50 tap-highlight-none no-select"
                    >
                        ...
                        <>
                          ...
                          >
                            ...
                            <button
                              ...
                              className="w-full text-left px-4 py-3 text-sm text-red-400 hover:bg-red-400/10 transition-colors flex items-center gap-2 min-h-touch tap-highlight-none"
                            >
            ...
            {/* Main Content */}
            <main className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 pt-16 sm:pt-20 md:pt-24 pb-8 md:pb-10">

              {/* Controls */}
              <div className="flex flex-col gap-3 sm:gap-4 md:gap-0 sm:flex-row justify-between items-stretch sm:items-center mb-4 sm:mb-6 md:mb-10">
                <motion.div
                  initial={{ opacity: 0, x: -20 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: 0.5 }}
                  className="flex items-center justify-center bg-[#252525] rounded-full shadow-lg p-1 border border-white/5"
                >
                  <button onClick={() => handleWeekChange('prev')} className="min-w-touch min-h-touch px-3 sm:px-4 py-2 hover:bg-[#333] rounded-full text-gray-400 hover:text-white transition-colors tap-highlight-none no-select">←</button>
                  <span className="px-4 sm:px-6 font-medium text-gray-200 text-sm sm:text-base whitespace-nowrap">
                    {t('app.weekOf')} <span className="text-[#6FB92D]">{currentWeekStart.toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</span>
                  </span>
                  <button onClick={() => handleWeekChange('next')} className="min-w-touch min-h-touch px-3 sm:px-4 py-2 hover:bg-[#333] rounded-full text-gray-400 hover:text-white transition-colors tap-highlight-none no-select">→</button>
                </motion.div>
                ...
                <motion.button
                  ...
                  className="group flex items-center justify-center min-h-touch bg-[#6FB92D] hover:bg-[#5da025] text-white px-6 sm:px-8 py-3 rounded-full shadow-[0_4px_20px_rgba(111,185,45,0.3)] transition-all tap-highlight-none no-select"
                >
                  <span className="text-xl mr-2 group-hover:rotate-12 transition-transform">✨</span>
                  <span className="font-semibold text-sm sm:text-base">{t('app.aiAutoPlan')}</span>
                </motion.button>
              </div>
                  ...
                  : ['monday', 'tuesday', 'wednesday', 'thursday', 'friday']
                   ...
                   return (
                     <motion.div
                       ...
                       className={`glass-panel rounded-2xl sm:rounded-3xl overflow-hidden flex flex-col transition-all duration-300 hover:shadow-xl hover:shadow-[#6FB92D]/10 hover:border-[#6FB92D]/30 ${isToday ? 'ring-2 ring-[#6FB92D] ring-offset-2 ring-offset-[#181818]' : ''}`}
                       style={{ minHeight: prefs.enableBreakfast ? '28rem' : '22rem' }}
                     >
                       <div className="bg-[#2A2A2A]/50 px-4 sm:px-5 py-3 sm:py-4 border-b border-white/5 flex justify-between items-baseline">
                         <h3 className={`font-bold text-base sm:text-lg ${isToday ? 'text-[#6FB92D]' : 'text-gray-300'}`}>{dayName}</h3>
                       </div>

                       <div className="flex-1 p-3 sm:p-4 space-y-3 sm:space-y-4 flex flex-col">
                          {mealsToShow.map(meal => {
                            const mealItem = dayItems.find(i => i.mealTime === meal.id);
                            const isOrdered = mealItem?.status === OrderStatus.ORDERED;
                            const slotKey = mealItem ? `${mealItem.date}-${mealItem.mealTime}` : '';
                            ...
                            return (
                              <motion.button
                                ...
                                className={`flex-1 min-h-touch rounded-xl sm:rounded-2xl border-2 border-dashed p-3 sm:p-4 flex flex-col justify-center items-center text-center relative transition-all duration-300 group tap-highlight-none ${
                                isSlotLoading
                                ...
                                <span className="absolute top-2 sm:top-3 left-2 sm:left-3 text-[9px] sm:text-[10px] font-bold text-gray-500 uppercase tracking-widest bg-[#252525] px-1.5 sm:px-2 py-0.5 rounded">{meal.label}</span>
                                {!isSlotLoading && canEditSlot(mealItem) && (
                                  <span className="absolute top-2 sm:top-3 right-2 sm:right-3 text-[9px] sm:text-[10px] text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity">{t('app.clickToEdit')}</span>
                                )}
                                {isSlotLoading ? (
                                  <MealSkeleton color={`${meal.color}/30`} />
                                ) : isOrdered ? (
                                  <div className="w-full">
                                    <div className={`text-xs sm:text-sm font-bold ${meal.text} line-clamp-2 mb-1`}>{mealItem.currentOrder?.name || t('app.ordered')}</div>
                                    <div className="text-[10px] sm:text-xs text-gray-400 group-hover:text-gray-200 transition-colors">{mealItem.currentOrder?.restaurantName}</div>
                                  </div>
                                ) : !canEditSlot(mealItem) ? (
                                  <span className="text-xs sm:text-sm text-gray-600">{t('app.orderClosed')}</span>
                                ) : (
                                  <span className="text-xs sm:text-sm text-gray-600 group-hover:text-gray-400">{t('app.clickToOrder')}</span>
                                )}
                              </motion.button>
                            );
                          })}
